...
class Xsql {
    ...
    constructor(sqlConfig, pool, sqliteDB, memoryDB, sqlite3DB) {
        this.sqlConfig = {};
        this.pool = {};
        this.metaDb = {};
        this.metaDb.tables = {};
    ...
    cacheExec(query, params, route, execstr = '') {
        ...
        const result = new Promise((resolve, reject) => {
            that.sqlite3DB.all(execstr).then(rows => {
                return resolve(rows);
            });
        });
        return result;
    ...
    parallelExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        try {
            ...
            if (type == 'local' || type == 'local.nacos') {
                ...
                this.sqlite3DB.exec('BEGIN TRANSACTION');
                this.sqlite3DB.exec(query);
                this.sqlite3DB.exec('COMMIT');
