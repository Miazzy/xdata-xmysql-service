...
class Xsql {
    ...
    constructor(sqlConfig, pool, sqliteDB, memoryDB) {
        this.sqlConfig = {};
        this.pool = {};
        this.metaDb = {};
        this.metaDb.tables = {};
                ...
                (async() => { //异步日志记录到MySQL中
                    ...
                    try {
                        if (route == 'create' || route == 'bulkInsert' || route == 'multiInsert' || route == 'bulkInsert') { //新增数据，由于新增数据时通过INSERT INTO SET语句执行的无法兼容其他数据库，需做特殊处理
                            ...
                            this.mergeExec(tableName, execstr);
    ...
    parallelExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        try {
            ...
            if (type == 'local' || type == 'local.nacos') {
                ...
                this.sqliteDB.query(query, params, callback);
                this.memoryDB.query(query, params, callback);
