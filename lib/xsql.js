...
class Xsql {
    ...
    cacheExec(query, params, route, execstr = '') {
        ...
        console.log(`cache exec  ${qType}.${qDatabase}.${qTableName} `);
        execstr = sqlstring.format(query, params);
        console.info(`cache exec >>>>>>>>>>>>>>>`, execstr);
    ...
    exec(query, params, route, execstr = '') {
        ...
        const result = new Promise((resolve, reject) => {
            console.log(`mysql>>>>> `, query, ` params: `, JSON.stringify(params));
            ...
            if ((route == 'read') && keys.includes(qTableName)) {
                (async() => {
                    ...
                    if (!tools.isNull(rows) && rows.length > 0) {
                        console.log(`query data by local sqlite & hit cache ... table :`, qTableName, `query sql :`, querySQL, ` rows :`, rows, ` hit cache ... `);
                        return resolve(rows);
                    }
                })();
            }
                ...
                (async() => { //异步日志记录到MySQL中
                    ...
                    try {
                        if (route == 'create' || route == 'bulkInsert' || route == 'multiInsert' || route == 'bulkInsert') { //新增数据，由于新增数据时通过INSERT INTO SET语句执行的无法兼容其他数据库，需做特殊处理
                            ...
                            that.mergeExec(tableName, execstr, qarams.params);
                        } else if (route == 'update' || route == 'patch' || route == 'multiPatch') { //修改数据
                            execstr = sqlstring.format(query, params);
                        ...
                        console.log(`create qarams error:`, error);
                    }
                })();
    ...
    parallelExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        ...
        try {
            ...
            if (type == 'local' || type == 'local.nacos') {
                console.log(`cache ddl [database].[table] >>>>>>>>>>>>>>>>>> :`, `${qType}.${qDatabase}.${qTableName}`);
                this.sqliteDBMap.get(`${qType}.${qDatabase}.${qTableName}`).run('BEGIN TRANSACTION');
                this.sqliteDBMap.get(`${qType}.${qDatabase}.${qTableName}`).run(query);
                this.sqliteDBMap.get(`${qType}.${qDatabase}.${qTableName}`).run('COMMIT');
                console.log(`cache ddl exec >>>>>>>>>>>>>>>>>> : `, query, `params: `, params);
            }
            ...
            if (type == 'nacos' || type == 'local.nacos') {
                ...
                for (const target of hosts) {
                    ...
                    console.log(`cache nacos keys >>>>>>>>>>>>>>>>>> :`, keys);
                    ...
                    console.log(`cache nacos exec >>>>>>>>>>>>>>>>>> : `, query, `params: `, params);
                }
            }
        } catch (error) {
            console.log(`parallel exec error: `, error);
        }
    }
