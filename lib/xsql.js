...
const dataHelp = require("./util/data.js");
const whereHelp = require("./util/analyzer.js");
 ...
 * @classdesc RestAPI SQL业务解析器
 * @author xmysql
 */
class Xsql {

    constructor(sqlConfig, pool, sqliteDB, memoryDB, sqliteDBMap) {
        this.sqlConfig = {};
        this.pool = {};
        this.metaDb = {};
        this.metaDb.tables = {};
    ...
    //设置RPC消费者Map
    setRpcConsumeMap(map = new Map()) {
        this.rpcConsumeMap = map;
    }

    //缓存数据库表信息 /**************** START : Cache functions ****************/
    dbCacheInitAsync(cbk) {
        let self = this;
        self.pool.query(
            dataHelp.getSchemaQuery(), [this.sqlConfig.database],
            (err, results) => {
                if (err) {
    ...
    //执行查询操作时，如果本地sqlite命中，则直接返回sqlite数据
    cacheExec(query, params, route, execstr = '') {
        const that = this;
        const qType = config().service.type || 'mysql';
        const qDatabase = config().service.database || 'xdata';
        const qTableName = `${params[0]}`;
        execstr = sqlstring.format(query, params);
        console.info(`cache exec  ${qType}.${qDatabase}.${qTableName} `, `cache exec >>>>>>>>>>>>>>>`, execstr);
        const result = new Promise((resolve, reject) => {
            that.sqliteDBMap.get(`${qType}.${qDatabase}.${qTableName}`).all(execstr).then(rows => {
                return resolve(rows);
            });
        });
        return result;
    ...
    //执行数据库操作
    exec(query, params, route, execstr = '') {
        const that = this;
        const qType = config().service.type || 'mysql';
        const qDatabase = config().service.database || 'xdata';
        const cache_sqlite_query = config().memorycache.cache_sqlite_query;
        const qTableName = `${params[0]}`;
        const tablenames = config().memorycache.cacheddl;
        const keys = Object.keys(tablenames);
        let hitCacheFlag = false;

        const result = new Promise((resolve, reject) => {
            console.info(`mysql>>>>> `, query, ` params: `, JSON.stringify(params));

            const sqliteDB = that.sqliteDBMap.get(`${qType}.${qDatabase}.${qTableName}`);
            const tsql = query.trim().toLowerCase();
            if (cache_sqlite_query && ('|read|list|count|'.includes(route)) && keys.includes(qTableName)) { // '|read|list|count|'.includes(route)
                (async() => {
                    const querySQL = sqlstring.format(query, params).trim();
                    const rows = await sqliteDB.all(querySQL);
                    console.log(`query sqlite `, qTableName, route, rows.length);
                    if (!tools.isNull(rows) && rows.length > 0) {
                        console.info(`query data by local sqlite & hit cache ... table :`, qTableName, `query sql :`, querySQL, ` rows :`, rows.length > 0 ? rows[0] : rows, ` hit cache ... `);
                        hitCacheFlag = true;
                        return resolve(rows);
                    }
                })();
            ...
            //执行主数据库CRUD[增删改查]操作
            that.pool.query(query, params, (error, rows, _fields) => {

                if (error) { //如果执行错误，则直接返回
                    console.info("mysql>>>>> ", error);
                    return reject(error);
                }

                //记录消息队列或者异步日志query, params
                (async() => { //异步日志记录到MySQL中
                    const [tableName, ...nodes] = params;
                    try {
                        if (route == 'create' || route == 'bulkInsert' || route == 'multiInsert' || route == 'bulkInsert') { //新增数据，由于新增数据时通过INSERT INTO SET语句执行的无法兼容其他数据库，需做特殊处理
                            const qarams = tools.parseInsertStatement(params[0], nodes, that.metaDb);
                            const statement = tools.parseInsertSQL(tableName, nodes, that.metaDb);
                            execstr = sqlstring.format(qarams.query, qarams.params);
                            // console.info(`insert sql:`, statement);
                            that.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, qarams.query, JSON.stringify(qarams.params), execstr, route]); //console.log(`qarams.table:`, tableName, `qarams.nodes:`, nodes); console.log(`qarams.query: `, qarams.query, `qarams.params:`, qarams.params);
                            that.mergeExec(tableName, statement.query.replace(/INSERT INTO/g, 'INSERT OR REPLACE INTO'), statement.params);
                        } else if (route == 'update') { //修改数据 REPLACE INTO
                            execstr = sqlstring.format(query, params);
                            that.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), execstr, route]);
                            // console.info(`update sql:`, execstr);
                            that.mergeExec(tableName, execstr.replace(/REPLACE INTO/g, 'INSERT OR REPLACE INTO'), []);
                        } else if (route == 'patch' || route == 'multiPatch') { //修改数据
                            execstr = sqlstring.format(query, params);
                            query = query.replace('??', tableName);
                            that.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), execstr, route]);
                            // console.info(`patch sql:`, query, nodes);
                            that.mergeExec(tableName, query, nodes);
                        } else if (route == 'delete' || route == 'multiDelete' || route == 'whereDelete' || route == 'whereDeleteAndInsert') { //删除数据
                            execstr = sqlstring.format(query, params);
                            that.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), execstr, route]);
                            // console.info(`delete sql:`, execstr);
                            that.mergeExec(tableName, execstr, []);
                        } else { //查询数据
                            execstr = sqlstring.format(query, params).trim();
                            that.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), execstr, route]);
                            if (!execstr.toUpperCase().includes('SELECT')) { // 如果不是查询语句，则需要执行增删改操作
                                that.mergeExec(tableName, execstr, params);
                            }
                        }
                    } catch (error) {
                        console.info(`create qarams error:`, error);
                    }
                })();

                if (hitCacheFlag == false) {
                    ('|read|list|count|'.includes(route)) && keys.includes(qTableName) ? console.log(`query data by remote database & rows `, rows.length > 0 ? rows[0] : rows, `not hit cache ... `) : null;
                    return resolve(rows);
                }
            });
        });
    ...
    //合并执行
    mergeExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        this.parallelExec(tableName, query, params, 'local.nacos', callback);
        this.nacosShareExec(tableName, query, params, type, callback);
        this.slaveDBExec(tableName, query, params, type, callback);
        this.mqueueExec(tableName, query, params, type, callback);
    }

    //本地数据库并发执行
    parallelExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        const qType = config().service.type || 'mysql';
        const qDatabase = config().service.database || 'xdata';
        const qTableName = `${tableName}`;
        const sqliteDB = this.sqliteDBMap.get(`${qType}.${qDatabase}.${qTableName}`);
        try {
            ...
            console.info(`parallel exec >>>>>>>>>>>>>>>>>> : `, query, `params: `, params, ` type:`, type);
            //执行本地sqlite写入工作
            if (type == 'local' || type == 'local.nacos') {
                console.info(`cache ddl [database].[table] >>>>>>>>>>>>>>>>>> :`, `${qType}.${qDatabase}.${qTableName}`);
                sqliteDB.run('BEGIN TRANSACTION');
                sqliteDB.run(query, params);
                sqliteDB.run('COMMIT');
                console.info(`cache ddl exec >>>>>>>>>>>>>>>>>> : `, query, `params: `, params);
            }
            //调用Nacos注册的服务的其他的parallelExec 如果通过微服务调用，不能再次触发调用Nacos微服务的parallelExec
            if (type == 'nacos' || type == 'local.nacos') {
                const nacosConfig = this.rpcConsumeMap.get('nacos.config');
                const hosts = this.rpcConsumeMap.get(nacosConfig.serviceName);
                const ipAddress = this.rpcConsumeMap.get('local.ipaddress');
                for (const target of hosts) {
                    if (!(target.valid && target.healthy && target.enabled)) { // 选出健康的targets;
                        continue;
                    }
                    if (target.ip === ipAddress) { //IP地址为本地则不用执行，因为本地已经执行
                        continue;
                    }
                    console.info(`cache nacos keys >>>>>>>>>>>>>>>>>> :`, keys);
                    const consumer = this.rpcConsumeMap.get(target.ip);
                    consumer.invoke('parallelExec', [tableName, query, params, 'local'], { responseTimeout: 3000 });
                    console.info(`cache nacos exec >>>>>>>>>>>>>>>>>> : `, query, `params: `, params);
                }
            }
        } catch (error) {
            console.info(`parallel exec error: `, error);
        }
    }

    //在Nacos注册的数据库分片执行
    nacosShareExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        try {
            //检查自己分片编号，查询分片此表分片规则，根据自己分片编号和分片规则进行匹配，如果匹配则执行操作

            //调用Nacos注册的服务的其他的nacosShareExec 如果通过微服务调用，不能再次触发调用Nacos微服务的nacosShareExec
            if (type == 'nacos') {

            }
        } catch (error) {
            console.log(`nacos share exec: `, error);
        }
    }

    //从数据库并发执行
    slaveDBExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        //检查是否配置有从数据库，如果有从数据库，则执行操作，从数据库可以MySQL、PostgreSQL、Oracle，从数据库无需调用Nacos注册的微服务
    }

    //消息队列异步执行
    mqueueExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        //检查消息队列是否配置，注意消息队列请设置消息保留时间10年或永久，消息队列无需调用Nacos注册的微服务
    }
    ...
    getLimitClause(reqParams) {
        ...
        if ("_size" in reqParams) {
            if (parseInt(reqParams._size) > 0 && parseInt(reqParams._size) <= 100000) {
                reqParams._len = parseInt(reqParams._size);
            } else if (parseInt(reqParams._size) > 100000) {
                reqParams._len = 100000;
            }
        }
        ...
        console.log(`_index: `, reqParams._index, ` _len: `, reqParams._len);
    ...
    prepareBulkReplace(tableName, objectArray, queryParamsObj) {
        if (tableName in this.metaDb.tables && objectArray) {
            let insObj = objectArray[0];

            // goal => replace into ?? (?,?..?) values ? [tablName, col1,col2...coln,[[ObjValues_1],[ObjValues_2],...[ObjValues_N]]
            queryParamsObj.query = "REPLACE INTO ?? ( ";
            queryParamsObj.params.push(tableName);
            ...
            // iterate over all column in table and have only ones existing in objects to be inserted
            for (
                var i = 0; i < this.metaDb.tables[tableName]["columns"].length;
                ++i
            ) {
            ...
            for (let i = 0; i < orderByCols.length; ++i) {
                ...
                if (orderByCols[i][0] === "-") {
                    ...
                    try {
                        ...
                        // queryParamsObj.params.push(mysql.escapeId(orderByCols[i].substring(1, len)));
                    }
                } else {
                    ...
                    try {
                        ...
                        // queryParamsObj.params.push(mysql.escapeId(orderByCols[i]));
                    }
                }
            }
        }
    ...
    getSchemaRoutes(internal, apiPrefix) {
        ...
        for (var tableName in this.metaDb.tables) {
            if (tableName in this.sqlConfig.ignoreTables) {
                ...
                //查询报表数据操作
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/describe", "describe")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/chart", "chart")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/aggregate", "aggregate")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/autoChart", "autoChart")
                );

                //查询操作
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/count", "count")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/groupby", "groupby")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/ugroupby", "ugroupby")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/distinct", "distinct")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/findOne", "findOne")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName, "list")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/bulk", "bulkRead")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/:id", "read")
                );
                routes.push(
                    this.prepareRoute(internal, "get", apiPrefix, tableName + "/:id/exists", "exists")
                );
                ...
                if (!isView && !this.sqlConfig.readOnly) {
                    ...
                    routes.push(
                        this.prepareRoute(internal, "delete", apiPrefix, tableName, "whereDelete")
                    );
                    routes.push(
                        this.prepareRoute(internal, "put", apiPrefix, tableName, "update")
                    );
                    routes.push(
                        this.prepareRoute(internal, "patch", apiPrefix, tableName + "/:id", "patch")
                    );
                    routes.push(
                        this.prepareRoute(internal, "delete", apiPrefix, tableName + "/:id", "delete")
                    );

                    //多数据操作 ，非只读模式(批量操作)
                    routes.push(
                        this.prepareRoute(internal, "post", apiPrefix, tableName + "/multi", "multiInsert")
                    );
                    routes.push(
                        this.prepareRoute(internal, "post", apiPrefix, tableName + "/bulk", "bulkInsert")
                    );
                    routes.push(
                        this.prepareRoute(internal, "delete", apiPrefix, tableName + "/bulk", "bulkDelete")
                    );
                    routes.push(
                        this.prepareRoute(internal, "patch", apiPrefix, tableName + '/bulk', "whereDeleteAndInsert")
                    );
                ...
                for (var j = 0; j < table["foreignKeys"].length; ++j) {
                    ...
                    if (fk["referenced_table_name"] in this.sqlConfig.ignoreTables) {
                        ...
                        routes.push(
                            this.prepareRoute(internal, "get", apiPrefix, fk["referenced_table_name"] + "/:id/" + fk["table_name"], "relational")
                        );
                    }
                }
    ...
    getChartQueryAndParamsFromStepPair(
        ...
        if (
            ...
            for (
                let i = 0; i < stepArray.length && stepArray.length >= 2; i = i + 2
            ) {
    ...
    prepareJoinQuery(req, res, queryParamsObj) {
        ...
        while (1) {
            ...
            for (
                let i = 0; i < joinTables.length - 1 && queryParamsObj.grammarErr === 0; i = i + 2
            ) {
                onCondnCount++;
