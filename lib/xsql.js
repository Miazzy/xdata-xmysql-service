...
class Xsql {
    ...
    //合并执行
    mergeExec(tableName = '', query = '', params = [], callback = () => {}) {
        this.parallelExec(tableName, query, params, callback);
        this.nacosShareExec(tableName, query, params, callback);
        this.slaveDBExec(tableName, query, params, callback);
        this.mqueueExec(tableName, query, params, callback);
    }

    //本地数据库并发执行
    parallelExec(tableName = '', query = '', params = [], callback = () => {}) {
        try {
    ...
    //在Nacos注册的数据库分片执行
    nacosShareExec(tableName = '', query = '', params = [], callback = () => {}) {
        try {
    ...
    //从数据库并发执行
    slaveDBExec(tableName = '', query = '', params = [], callback = () => {}) {
        //检查是否配置有从数据库，如果有从数据库，则执行操作，从数据库可以MySQL、PostgreSQL、Oracle，从数据库无需调用Nacos注册的微服务
    }

    //消息队列异步执行
    mqueueExec(tableName = '', query = '', params = [], callback = () => {}) {
