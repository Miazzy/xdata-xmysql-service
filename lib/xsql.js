    ...
    parallelExec(tableName = '', query = '', params = [], type = 'local', callback = () => {}) {
        ...
        try {
            ...
            if (type == 'local' || type == 'local.nacos') {
                ...
                this.sqliteDBMap.get(`${type}.${database}.${qTableName}`).run('BEGIN TRANSACTION');
                this.sqliteDBMap.get(`${type}.${database}.${qTableName}`).run(query);
                this.sqliteDBMap.get(`${type}.${database}.${qTableName}`).run('COMMIT');
