    ...
    exec(query, params, route) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            (async() => { //异步日志记录到MySQL中
                ...
                console.log(`qarams.table:`, tableName, `qarams.nodes:`, nodes);
                try {
                    if (route == 'create' || route == 'bulkInsert') {
                        const qarams = tools.parseInsertStatement(params[0], nodes, _this.metaDb);
                        console.log(`qarams.query: `, qarams.query, `qarams.params:`, qarams.params);
                        _this.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, qarams.query, JSON.stringify(qarams.params), sqlstring.format(qarams.query, qarams.params), route]);
                    } else {
                        _this.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), sqlstring.format(query, params), route]);
                    }
                } catch (error) {
