        ...
        const result = new Promise((resolve, reject) => {
            ...
            if (cache_sqlite_query && ('|read|list|count|'.includes(route)) && keys.includes(qTableName)) {
                (async() => {
                    ...
                    console.log(`query sqlite `, qTableName, route, rows);
                    if (!tools.isNull(rows) && rows.length > 0) {
                        console.info(`query data by local sqlite & hit cache ... table :`, qTableName, `query sql :`, querySQL, ` rows :`, rows, ` hit cache ... `);
                        return resolve(rows);
                    }
                })();
            }
