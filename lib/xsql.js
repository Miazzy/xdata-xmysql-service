            ...
            (async() => { //异步日志记录到MySQL中
                ...
                try {
                    if (route == 'create' || route == 'bulkInsert' || route == 'multiInsert' || route == 'bulkInsert') { //新增数据，由于新增数据时通过INSERT INTO SET语句执行的无法兼容其他数据库，需做特殊处理
                        const qarams = tools.parseInsertStatement(params[0], nodes, _this.metaDb);
                        execstr = sqlstring.format(qarams.query, qarams.params);
                        _this.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, qarams.query, JSON.stringify(qarams.params), execstr, route]); //console.log(`qarams.table:`, tableName, `qarams.nodes:`, nodes); console.log(`qarams.query: `, qarams.query, `qarams.params:`, qarams.params);
                    } else if (route == 'update' || route == 'patch' || route == 'multiPatch' || route == 'delete' || route == 'multiDelete' || route == 'whereDelete' || route == 'whereDeleteAndInsert') {
                        execstr = sqlstring.format(query, params);
                        _this.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), execstr, route]);
                    } else {
                        execstr = sqlstring.format(query, params);
                        _this.pool.query(`INSERT INTO bs_sql_log (id, tname, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?, ?);`, [tools.queryUniqueID(), tableName, query, JSON.stringify(params), execstr, route]);
                    }
