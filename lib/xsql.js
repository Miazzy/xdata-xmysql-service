...
class Xsql {
    ...
    exec(query, params, route, execstr = '') {
        ...
        const result = new Promise((resolve, reject) => {
            ...
            if (cache_sqlite_query && ('|read|'.includes(route)) && keys.includes(qTableName)) { // '|read|list|count|'.includes(route)
                (async() => {
            ...
            that.pool.query(query, params, (error, rows, _fields) => {
                ...
                if (hitCacheFlag == false) {
                    ('|read|'.includes(route)) && keys.includes(qTableName) ? console.log(`query data by remote database & rows `, rows.length > 0 ? rows[0] : rows, `not hit cache ... `) : null;
                    return resolve(rows);
                }
            });
        });
