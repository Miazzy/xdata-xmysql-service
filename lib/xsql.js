        ...
        const result = new Promise((resolve, reject) => {
            ...
            if (cache_sqlite_query && ('|read|list|count|'.includes(route)) && keys.includes(qTableName)) {
                (async() => {
                    ...
                    if (!tools.isNull(rows) && rows.length > 0) {
                        console.info(`query data by local sqlite & hit cache ... table :`, qTableName, `query sql :`, querySQL, ` rows :`, rows.length, ` hit cache ... `);
                        hitCacheFlag = true;
                        return resolve(rows);
                    }
                })();
            ...
            that.pool.query(query, params, (error, rows, _fields) => {
                ...
                if (hitCacheFlag == false) {
                    console.log(`query data by remote database & rows `, rows, `not hit cache ... `);
                    return resolve(rows);
                }
            });
        });
