            ...
            (async() => { //异步日志记录到MySQL中
                ...
                try {
                    if (route == 'create' || route == 'bulkInsert') {
                        ...
                        _this.pool.query(`INSERT INTO bs_sql_log (id, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?);`, [tools.queryUniqueID(), qarams.query, JSON.stringify(qarams.params), sqlstring.format(qarams.query, qarams.params), route]);
                    } else {
                        _this.pool.query(`INSERT INTO bs_sql_log (id, statement, params, execstr, route) VALUES (?, ?, ?, ?, ?);`, [tools.queryUniqueID(), query, JSON.stringify(params), sqlstring.format(query, params), route]);
                    }
                } catch (error) {
