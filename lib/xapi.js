...
class Xapi {
    ...
    uploadFile(req, res) {
        if (req.file) { // console.log(req.file.path);
            res.json({ err: false, success: true, path: req.file.path, name: req.file.path.split('xdata-xmysql-service/')[1], cwd: process.cwd() });
        } else {
    ...
    uploadFiles(req, res) {
        if (!req.files || req.files.length === 0) {
            ...
            res.json({ err: false, success: true, path: files.toString(), cwd: process.cwd() });
        }
    }
