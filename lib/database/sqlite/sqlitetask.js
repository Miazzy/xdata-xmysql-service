            ...
            for await (tableName of keys) { // 根据配置参数选择，增量查询或者全量查询
                ...
                if (flag == `true` && fileFlag) { /***************** 方案一 增量 *****************/
                    try {
                        ...
                        insync(qTableName, sqliteDBMap, pool); //查询本地sqlite数据，获取当前最大值 id , xid //查询主数据库数据库大于当前最大值的数据 id 新增 //将多的数据同步新增过来
                    } catch (error) {
                    ...
                    try {
                        ...
                        sync(qTableName, sqliteDBMap, pool);
                    } catch (error) {
...
const insync = async(qTableName, sqliteDBMap, pool) => {
    ...
    // 查询MaxID,MinID
    let querySQL = `select max(id) maxid , min(id) minid from ${qTableName} `; //需要检查ID是否存在
    console.log(`exec #sync# tablename#${qTableName}# >>>>>>>>>>>>>> :`, ` select sql :`, querySQL);
    const sqliteDB = sqliteDBMap.get(`${type}.${database}.${qTableName}`);
    const tempRows = await sqliteDB.all(querySQL);
    const { maxid, minid } = tempRows[0];
...
const sync = async(qTableName, sqliteDBMap, pool) => {
                    ...
                    while (page <= maxPage) {
                        try {
                            ...
                            if (tools.isNull(elem)) { //本地sqlite不存在，则继续插入操作
                                ...
                                console.log(`${type}.${database}.${qTableName} rows which id eq `, elem.id, ` is already existed ... `);
                            }
                        } catch (error) {
                            console.log(`sqlite db exec error:`, error);
                        } finally {
