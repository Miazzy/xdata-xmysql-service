...
class xctrl {
    ...
    async list(req, res) {
        ...
        //记录消息队列或者异步日志query,params
        (async() => { //异步日志记录到MySQL中
            await this.mysql.exec(`INSERT INTO bs_sql_log (id, statement, params) VALUES (?, ?, ?);`, [tools.queryUniqueID(), queryParamsObj.query, JSON.stringify(queryParamsObj.params)]);
        })();
    ...
    async chart(req, res) {
        ...
        if (req.query) {
            ...
            //记录消息队列或者异步日志query,params
            (async() => { //异步日志记录到MySQL中
                await this.mysql.exec(`INSERT INTO bs_sql_log (id, statement, params) VALUES (?, ?, ?);`, [tools.queryUniqueID(), obj.query, JSON.stringify(obj.params)]);
            })();
