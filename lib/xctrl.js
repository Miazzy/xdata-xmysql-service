...
class xctrl {
    ...
    async list(req, res) {
        ...
        if (queryParamsObj && results) {
            console.log(`list query: `, queryParamsObj.query);
            console.log(`list params: `, JSON.stringify(queryParamsObj.params));
            //缓存查询query和params,如果3s内再次查询则使用缓存
    ...
    async read(req, res) {
        ...
        if (results) {
            //缓存查询query和params,如果5s内再次查询则使用缓存
