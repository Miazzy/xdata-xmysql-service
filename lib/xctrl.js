...
class xctrl {
    ...
    //构造函数
    constructor(app, mysql, sqliteDB, memoryDB, sqliteDBMap) {
        this.app = app;
        this.mysql = mysql; //此处的mysql是new Xsql(...)的Instance，xapi.js 构造器中 this.mysql = new Xsql(args, mysqlPool, sqliteDB, memoryDB);
    ...
    async create(req, res) {
        ...
        (req.body.xid || req.headers.xid) ? (req.body.xid = tools.queryUniqueID()) : null; // console.log(`insert request body:`, req.body, ` req.headers.xid:`, req.headers.xid);
        const results = await this.mysql.exec(query, params, 'create');
    ...
    async list(req, res) {
        ...
        try {
            results = await getValue(key); //查询到缓存信息，直接返回
            if (results) {
                results = typeof results == 'string' ? JSON.parse(results) : results; // console.log(`result : ${JSON.stringify(results).slice(0,10)}`);
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'list');

        try { //设置缓存 tablename, query&params, results 
            if (results) { // console.log(`cache key: ${key}`); //缓存查询query和params,如果3s内再次查询则使用缓存
                redis.set(key, JSON.stringify(results), 'EX', serviceConfig.commonCacheTime || 1.5);
            }
        } catch (error) {
            console.log(error);
        }
    ...
    async nestedList(req, res) {
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'nestedList');
    ...
    async findOne(req, res) {
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'findOne');
    ...
    async read(req, res) {
        ...
        results = await this.mysql.exec(query, params, 'read');

        //设置缓存 tablename, query&params, results 
        try {
            if (results) { // console.log(`cache key: ${key}`); //缓存查询query和params,如果5s内再次查询则使用缓存
                redis.set(key, JSON.stringify(results), 'EX', serviceConfig.commonCacheTime || 1.5);
            }
        } catch (error) {
            console.log(error);
    ...
    async exists(req, res) {
        ...
        results = await this.mysql.exec(query, params, 'exists');
    ...
    async update(req, res) {
        ...
        var results = await this.mysql.exec(query, params, 'update');
        results.node = req.body;
    ...
    async patch(req, res) {
        ...
        let results = await this.mysql.exec(query, params, 'patch');
        results.node = req.body;
    ...
    async delete(req, res) {
        ...
        let results = await this.mysql.exec(query, params, 'delete');
    ...
    async multiPatch(req, res, tableName, patchID = '') {
        ...
        if (clause) {
            ...
            await this.mysql.exec(query, params, 'multiPatch');
    ...
    async multiDelete(req, res, ftableName = '', tableName = '') {
        ...
        try {
            ...
            await this.mysql.exec(query, params, 'multiDelete');
    ...
    async multiInsert(req, res) {
        ...
        try {
            ...
            for (let tableName of tableNames) {
                ...
                results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'multiInsert');
    ...
    async bulkInsert(req, res) {
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'bulkInsert');
    ...
    async whereDelete(req, res) {
        ...
        var results = await this.mysql.exec(query, params, 'whereDelete');
    ...
    async whereDeleteAndInsert(req, res) {
        ...
        params.push(req.app.locals._tableName);
        // console.log(`query:${query}, params:${JSON.stringify(params)}`);
        await this.mysql.exec(query, params, 'whereDeleteAndInsert');
        //设置缓存 tablename, key&value , status#delete , delete操作需要先查询一次
    ...
    async bulkDelete(req, res) {
        ...
        var results = await this.mysql.exec(query, params, 'bulkDelete');
    ...
    async bulkRead(req, res) {
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'bulkRead');
    ...
    async count(req, res) {
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'count');
    ...
    async distinct(req, res) {
        ...
        results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'distinct');
    ...
    async groupBy(req, res) {
        if (req.query && req.query._fields) {
            ...
            var results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'groupBy');
            res.status(200).json(results);
        } else {
    ...
    async ugroupby(req, res) {
        if (req.query && req.query._fields) {
            ...
            //console.log(queryParamsObj.query, queryParamsObj.params);
            var results = await this.mysql.exec(queryParamsObj.query, queryParamsObj.params, 'ugroupby');
    ...
    async aggregate(req, res) {
        if (req.query && req.query._fields) {
            ...
            var results = await this.mysql.exec(query, params, 'aggregate');
    ...
    async chart(req, res) {
        ...
        if (req.query) {
            ...
            if (req.query && req.query.min && req.query.max && req.query.step) {
                ...
                let results = await _this.mysql.exec(query, params, 'chart_sub');
            ...
            let results = await this.mysql.exec(obj.query, obj.params, 'chart');
    ...
    async autoChart(req, res) {
        ...
        let describeResults = await this.mysql.exec(query, params, 'autoChart');
        ...
        for (var i = 0; i < describeResults.length; ++i) {
            if (
                ...
                let r = await this.mysql.exec(obj.query, obj.params, 'auto_chart_obj');
