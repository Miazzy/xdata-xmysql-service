...
class xctrl {
    ...
    async multiInsert(req, res) {
        try {
            let query = `delete from :ftableName where :fieldName = ':fieldValue' `;
            let params = [];
            let ftableName = req.query.ftableName || '-';
        ...
        try {
            let query_ = `delete from :ftableName_ where :fieldName_ = ':fieldValue_' `;
            let params_ = [];
            let ftableName_ = req.query.ftableName_ || '-';
            let fieldName_ = req.query.fieldName_ || '-';
            let fieldValue_ = req.query.fieldValue_ || '-';
            query_ = query_.replace(':ftableName_', ftableName_).replace(':fieldName_', fieldName_).replace(':fieldValue_', fieldValue_);
            params_.push(ftableName_);
            await this.mysql.exec(query_, params_);
        } catch (error) {
            console.log(error);
        }
        try {
            let query__ = `delete from :ftableName__ where :fieldName__ = ':fieldValue__' `;
            let params__ = [];
            let ftableName__ = req.query.ftableName__ || '-';
            let fieldName__ = req.query.fieldName__ || '-';
            let fieldValue__ = req.query.fieldValue__ || '-';
            query__ = query__.replace(':ftableName__', ftableName__).replace(':fieldName__', fieldName__).replace(':fieldValue__', fieldValue__);
            params__.push(ftableName__);
            await this.mysql.exec(query__, params__);
        } catch (error) {
            console.log(error);
        }

        try {
            multiPatch(req, res, req.query.ptableName, req.query.patchID);
        } catch (error) {
            console.log(error);
        }
        ...
        let queryParamsObj = {};
        let results = [];
        let tableNames = req.app.locals._tableName.split(',');
        queryParamsObj.query = "";
        queryParamsObj.params = [];
