...
function isNull(data) {
    try {
        if (typeof data == 'undefined' || data == null || data == undefined || data == '' || JSON.stringify(data) == "{}") {
            return true;
        } else {
            return false;
        }
 ...
 * @returns 
 */
function parseInsertStatement(tableName, objectArray, metaDb, queryParamsObj = { query: '', params: [] }) {

    if (tableName in metaDb.tables && objectArray) {
        ...
        let cols = [];
        let colPresent = false; //是否在字段前添加前缀逗号

        // iterate over all column in table and have only ones existing in objects to be inserted
        for (var i = 0; i < metaDb.tables[tableName]["columns"].length; ++i) {
            let colName = metaDb.tables[tableName]["columns"][i]["column_name"];
 ...
 * @returns 
 */
function parseInsertSQL(tableName, objectArray, metaDb, queryParamsObj = { query: '', params: {} }) {
    if (tableName in metaDb.tables && objectArray) {
        const insObj = objectArray[0];
        queryParamsObj.query = `INSERT INTO ${tableName} ( `;
        let colPresent = false; //是否在字段前添加前缀逗号
        for (var i = 0; i < metaDb.tables[tableName]["columns"].length; ++i) {
            let colName = metaDb.tables[tableName]["columns"][i]["column_name"];
            if (colName in insObj) {
                colPresent ? (queryParamsObj.query += ",") : null;
                queryParamsObj.query += colName;
                colPresent = true;
            }
        }
        ...
        for (var i = 0; i < metaDb.tables[tableName]["columns"].length; ++i) {
            ...
            if (colName in insObj) {
                colPresent ? (queryParamsObj.query += ",") : null;
                queryParamsObj.query += '@' + colName;
                queryParamsObj.params['@' + colName] = insObj[colName];
                colPresent = true;
            }
        }
        queryParamsObj.query += " ) ";
 ...
 * @returns 
 */
function parseUpdateSQL(tableName, objectArray, metaDb, queryParamsObj = { query: '', params: {} }) {
    if (tableName in metaDb.tables && objectArray) {
        const insObj = objectArray[0];
        queryParamsObj.query = `UPDATE ${tableName} SET `;
        let colPresent = false; //是否在字段前添加前缀逗号
        for (var i = 0; i < metaDb.tables[tableName]["columns"].length; ++i) {
            let colName = metaDb.tables[tableName]["columns"][i]["column_name"];
            if (colName in insObj) {
        ...
        queryParamsObj.query += ` WHERE id = '${insObj.id}' `;
    }
    return queryParamsObj;
}
